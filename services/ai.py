...
class AIService:
    ...
    def __init__(
        ...
        if not self.active:
            logger.warning(
                "No AI providers (Gemini/Groq) are correctly configured. AI features disabled."
            )
    ...
    async def analyze_user_taste(
        ...
        movie_entries = [
            f"- {m.get('title')}: {m.get('overview', 'No overview available.')}"
        ...
        if self.gemini_active:
            try:
                ...
                logger.warning(
                    f"All Gemini keys failed or error occurred: {e}. Trying Groq fallback..."
                )
