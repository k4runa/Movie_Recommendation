        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-6">
            ...
            <div className="space-y-2">
              <h2 className="text-3xl font-black tracking-tighter text-foreground">CineWave Eco</h2>
            ...
            <div className="grid grid-cols-1 gap-2 w-full max-w-[300px]">
              <button onClick={() => setValue("Recommend me an action movie")} className="text-xs font-bold py-2 px-4 rounded-xl border border-border hover:bg-accent transition-all">"Recommend me an action movie"</button>
              <button onClick={() => setValue("What are my favorite movies?")} className="text-xs font-bold py-2 px-4 rounded-xl border border-border hover:bg-accent transition-all">"What are my favorite movies?"</button>
            </div>
          </div>
        )}
          ...
          {messages.map((msg, i) => (
            ...
            >
              <div className={cn(
                ...
                msg.role === "user" ? "bg-primary/20 border-primary/30" : "bg-accent border-border"
              )}>
                {msg.role === "user" ? <User className="w-4 h-4 text-primary" /> : <Sparkles className="w-4 h-4 text-muted-foreground" />}
              </div>
              <div
                className={cn(
        ...
        <div className="relative group bg-accent/30 rounded-3xl border border-border/50 shadow-inner transition-all focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/40">
          ...
          <div className="flex items-center justify-between px-4 pb-4">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-50">Press Enter to send</span>
            </div>
            <button
              onClick={handleSend}
